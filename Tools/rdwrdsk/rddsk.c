...
int OpenDisk(char *DiskName)
{
  ...
  if ((res=open(DiskName,O_BINARY|O_RDONLY))==-1)
    {
      PError(DiskName);
      exit(2);
    }
...
void ReadFile(DirEntry msxf)
{
  ...
  if ( (fd=open(name,O_BINARY|O_CREAT|O_WRONLY|O_TRUNC,0666))==-1)
    {
      PError(name);
      return;
    }
  ...
  while(size && (curcl>=2) && (curcl<=maxcl) )
	...
	read(diskim, clusbuf, cluslen);
        write(fd,clusbuf, (size>cluslen)?cluslen:size);
