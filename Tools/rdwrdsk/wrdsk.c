...
{ DirEntry res;
  ...
  if ((file=open(name,O_RDONLY,0))==-1)
    {
	...
	setsh(res.d_date,t);
      ...
      size=0; prevcl=0; 
      while( (bsiz=read(file, clusbuf, cluslen)) && (curcl<= maxcl) )
	{
	  size+=bsiz;
	  lseek(diskim, data_ofs+(curcl-2)*cluslen, SEEK_SET);
	  write(diskim, clusbuf, bsiz);
...
{ 
  ...
  if ((res=open(DiskName,O_BINARY|O_RDWR))==-1)
    {
      if ((res=open(DiskName,O_BINARY|O_CREAT|O_RDWR,0666))==-1)
	{ 
      ...
      FAT=(byte*)xalloc(seclen*BootBlock[0x16]);
      ...
      || BootBlock[0x0D]!=cluslen/seclen )
	{
      ...
      FAT=(byte*)xalloc(seclen*BootBlock[0x16]);
      read(res,FAT,seclen*BootBlock[0x16]);
    }
  ...
  if (lseek(res,0,SEEK_END)==512)
  {
    ...
    for(pos=dir_ofs; pos<data_ofs; pos+=seclen) 
      write(res,clusbuf,seclen); /* clear directory */
    lseek(res,data_ofs+(maxcl-1)*cluslen-seclen, SEEK_SET);
    write(res,clusbuf,seclen); /* set file size */
  }
  else 
    puts("(add)");
  return res;
...
void CloseDisk(int img, char * DiskName)
{
  ...
  for( i=0; i<BootBlock[0x10]; i++)
    write(img,FAT,seclen*BootBlock[0x16]);
  if (close(img))
    PError(DiskName);
}
	  ...
	  if (dirbuf.d_fname[0])
	    {
	      ...
	      write(diskim,(char *)&dirbuf,sizeof(DirEntry));
	    }
	}
      else
